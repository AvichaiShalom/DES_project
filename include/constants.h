...
void encrypt_file_CTR(const char *input_file, const char *output_file, uint64_t key) {
	...
	memcpy(&nonce, nonceArr, INT_32_BITS);

	fwrite(&nonce, INT_32_BITS, 1, output);
...
void decrypt_file_CTR(const char *input_file, const char *output_file, uint64_t key) {
	...
	fread(&nonce, INT_32_BITS, 1, input);

	fseek(input, 0, SEEK_END);
	numOfBlocks = (ftell(input) - INT_32_BITS) / BLOCK_SIZE_BYTES;
	fseek(input, INT_32_BITS, SEEK_SET);
